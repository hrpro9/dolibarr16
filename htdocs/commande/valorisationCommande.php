...
if ($object->id > 0) {
    ...
    $orders = array();
    $sql = "SELECT p.ref, det.tva_tx, det.qty, det.price, s.reel, en.ref as entrepot FROM llx_commandedet as det";
    ...
    if ($res->num_rows) {
        while ($row = $res->fetch_assoc()) {
            array_push($orders, [$row['ref'], $row['price'], $row['qty'], $row['reel'],  $row['tva_tx'], $row['entrepot']]);
        }
    }
    ...
    print dol_get_fiche_head($head, 'valorisationCommande', $langs->trans("CustomerOrder"), -1, 'order');
    ...
    foreach ($orders as $key => $value) {
        $quantiteLivrer = $value[3] >= $value[2] ? $value[2] : $value[2] - $value[3];
        print '<tr>';
        print '<td>'.$value[0]. '</td>';
        print '<td>'.$value[1]. '</td>';
        print '<td>'.$value[2]. '</td>';
        print '<td>'.$value[3].'</td>';
        print '<td>'.($quantiteLivrer * $value[1]).'</td>';
        print '<td>'.intval($value[4]). '%</td>';
        print '<td>'.$value[5]. '</td>';
        print '</tr>';
    }
