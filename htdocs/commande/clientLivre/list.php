...
$idClientToEdit = GETPOST('id_client', 'int');
...
if($action == 'delete'){
	$error = 0;	
	$id_client = GETPOST('id', 'int');
	$db->begin();
	if (!$error) {   
		
		$sql = "update llx_client_livre set deleted = 1,  deleted_by = $user->id, deleted_at=now() where id = $id_client";
		$result = $db->query($sql);
		if ($result){
			$db->commit();
			$action = '';
			...
			$db->rollback();
			$action = 'view';
			setEventMessages("Veuillez réessayer client n'est pas Supprimer", null, 'errors');
		}
	...
	$db->begin();
	if (!$error) {	
		$sql = "insert into llx_client_livre (firstname,lastname, adresse, created_at, created_by) VALUES('$firstname', '$lastname', '$adresse',  NOW(), $user->id)";
		$result = $db->query($sql);
		if ($result){
			$db->commit();
			$action = '';
			...
			$db->rollback();
			$action = 'create';
			setEventMessages("Veuillez réessayer client n'est pas creé", null, 'errors');
		}
	...
	if (!$error) {
		...
		$sql = "UPDATE llx_client_livre SET adresse = '$adresse', firstname = '$firstname', lastname = '$lastname' WHERE id = '$id'";
		$result = $db->query($sql);
		if ($result){
			$db->commit();
			$action = '';
			...
			$db->rollback();
			$action = '';
			setEventMessages("Veuillez réessayer client n'est pas modifié", null, 'errors');
		}
...
llxHeader('', "Client à livrer", "");


$clients = array();
$sql = "select * from llx_client_livre  where deleted = 0";
$res = $db->query($sql);
if ($res->num_rows) {
    while ($row = $res->fetch_assoc()) {
        array_push($clients, [$row['id'], $row['firstname'], $row['lastname'], $row['adresse']]);
    }
}


print "<h2 style='margin-bottom: 5px;'>Client à livrer</h2>";
...
foreach ($clients as $key => $value) {
    ...
    if($idClientToEdit == $value[0]){
        ...
        print '
			...
			<td><input name="adresse" type="text" value="'.$value[3].'" required></td>';
        print '
            <td>
        ...
        print '<input type="hidden" name="token" value="' . newToken() . '">';
        print '<input type="hidden" name="id_client" value="' . $value[0] . '">';
        ...
        <button id="showedit" type="button" class="btnEdit" value="Modifier" onclick="editClient(\''.$value[0].'\')"><i class="fa fa-pencil"></i></button></td></form>';
    }
    print '</tr>';
}
print '</div>';
...
<script>
    function deleteDriver(id, nom) {
        ...
        $("#deleteConfirmation .confirmation-title").html("<h2 style='font-size: 18px;'>Voulez-vous supprimer client'" + nom + "'</h2>");
	...
	function cancel() {
		window.location.href = '/commande/clientLivre/list.php';
	}
