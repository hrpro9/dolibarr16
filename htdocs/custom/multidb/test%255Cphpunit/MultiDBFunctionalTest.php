...
/**
 * \file    test/phpunit/MultiDBFunctionalTest.php
 * \ingroup multidb
 ...
 * @package Testmultidb
 */
class MultiDBFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	{
		$this->url('/custom/multidb/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('multidb/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/multidb/admin/about.php');
		$this->authenticate();
		return $this->assertContains('multidb/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('multidbwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMultiDB_MultiDBTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMultiDB_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
