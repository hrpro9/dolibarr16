...
if($returnpage=="Autrecreditbail")
{ 
    $url = DOL_URL_ROOT . '/custom/etatscomptables/AutrescreditBail/declarationAutrescreditBail.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if( $check == 'true')
  {
    ...
    for ($i = 0; $i <= 248; $i++) {
        ${'autrecreditBail' . $i} = $_POST['autrecreditBail' . $i];
        if(${'autrecreditBail' . $i}==$autrecreditBail248)
        {

            $selectedDate = new DateTime($autrecreditBail248);
            $year = $selectedDate->format('Y');
            $data .= '$autrecreditBail' . $i . ' = ' . $year . ";\n";   

        }
        else if (is_string(${'autrecreditBail' . $i})) {
            // If the value is a string, add double quotes around it
            $data .= '$autrecreditBail' . $i . ' = "' . ${'autrecreditBail' . $i} . "\";\n";
        }
        else {
            $data .= '$autrecreditBail' . $i . ' = ' . ${'autrecreditBail' . $i} . ";\n";
        }
    ...
    $nomFichier = 'autrecreditBail_fichier_'. $year.'.php';
...
<html>
  ...
  <body>
<center>
<form method="POST" action="declarationAutrescreditBail.php">
    <?php
    // Loop to create the hidden input fields
    for ($i = 0; $i <= 248; $i++) {
        $autrecreditBail = ${'autrecreditBail' . $i};
      echo '<input type="hidden" name="autrecreditBail' . $i . '" value="' . $autrecreditBail . '" />';
    }
    ?>     
    <div>
    ...
    <table border="0" cellpadding="0" cellspacing="0" id="sheet0" class="sheet0 gridlines">
        ...
        <tbody>
          ...
          <tr class="row5">
            <?php
                for ($i = 0; $i <= 13; $i++) {
                    echo '<td class="column' . ($i + 2) . ' style16 null">'.${'autrecreditBail' . $i}.'</td>' . "\n";
                }
            ?>
          ...
          <tr class="row24">
            ...
            <td class="column9 style47 f"><?php echo $sum1;?></td>
            <td class="column10 style47 f"><?php echo $sum2;?></td>
