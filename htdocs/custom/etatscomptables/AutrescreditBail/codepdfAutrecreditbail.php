...
class pdf_Autrecreditbail extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = "", $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . \'/billanLaisse/Autrecreditbail/\';
			
				$year=GETPOST(\'year\');
			
				$file = $dir . "/Autrecreditbail".$year . ".pdf";
				// $file = $dir . "/Passif.pdf";
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset(\'CREDIT-BAIL\'));
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont(\'\', \'B\', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "TABLEAU DES LOCATIONS ET BAUX AUTRES QUE LE CREDIT-BAIL ".$year, 0, "C");
