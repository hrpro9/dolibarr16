<?php
    ...
    if($returnpage=="Etatchangment")
    { 
        $url = DOL_URL_ROOT . '/custom/etatscomptables/EtatChangements/declarationEtatChangement.php';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if( $check == 'true')
        {
            ...
            for ($i = 0; $i <= 46; $i++) {
                ${'EtatChangment' . $i} = $_POST['EtatChangment' . $i];
                if(${'EtatChangment' . $i}==$EtatChangment46)
                {
            ...
            $nomFichier = 'EtatChangment_fichier_'. $year.'.php';
...
<html>
  ...
  <body>
  <center>
    <form method="POST" action="declarationEtatChangement.php">
        <?php
        // Loop to create the hidden input fields
        for ($i = 0; $i <= 46; $i++) {
            $EtatChangment = ${'EtatChangment' . $i};
        echo '<input type="hidden" name="EtatChangment' . $i . '" value="' . $EtatChangment . '" />';
        }
        ?>     
        <div>
    ...
    <table border="0" cellpadding="0" cellspacing="0" id="sheet0" class="sheet0 gridlines">
        ...
        <tbody>
          ...
          <tr class="row3">
            ...
            <?php
            for ($i = 0; $i <=1; $i++) {
                echo '<td class="column1 style14 null">'.${'EtatChangment' . $i}.'</td>' . "\n";
            }                
            ?>
          </tr>
          <tr class="row4">
