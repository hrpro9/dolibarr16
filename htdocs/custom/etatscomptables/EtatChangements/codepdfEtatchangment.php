...
class pdf_Etatchangment extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = "", $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . \'/billanLaisse/Etatchangment/\';
			
				$year=GETPOST(\'year\');
			
				$file = $dir . "/Etatchangment".$year . ".pdf";
				// $file = $dir . "/Passif.pdf";
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset(\'Etat Changment\'));
			...
			<tr class="row2">
				<td class="column0 style11 s">Nature des Changements</td>
				<td class="column2 style12 s">Justification des Changements</td>
				<td class="column3 style12 s">Influence des Changements sur le Patrimoine, la Situation Financière et les Résultats</td>
			</tr>\';	
			
			$table .= \'<tr class="row3">
			<td class="column0 style14 null">I. Changements affectant les méthodes d évaluation</td>
			\';
			for ($i = 0; $i <= 1; $i++) {
			...
			$table .= \'<tr class="row5">
			<td class="column0 style14 null">II. Changements affectant les régles de présentation</td>
			\';
			for ($i = 23; $i <= 24; $i++) {
				$table .= \'<td class="column\' . ($i + 2) . \' style16 null">\'.${\'EtatChangment\' . $i}.\'</td>\' . "\n";
			}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont(\'\', \'B\', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "ETAT DES CHANGEMENTS DE METHODES ".$year, 0, "C");
