...
llxHeader("", "");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        for ($i = 0; $i <= 46; $i++) {
            ${'EtatChangment' . $i} = $_POST['EtatChangment' . $i];
        }
        ...
        function GenerateDocuments()
        {
            ...
            print '<input type="hidden" name="model" value="Etatchangment">';
        ...
        function ShowDocuments()
        {
            ...
            $subdir ='';
            $filedir = DOL_DATA_ROOT . '/billanLaisse/Etatchangment/';
            ...
            if ($societe !== null && isset($societe->default_lang)) {
                print $formfile->showdocuments('Etatchangment', $subdir, $filedir, $urlsource, $genallowed, $delallowed, $modelpdf, 1, 0, 0, 40, 0, '', '', '', $societe->default_lang);
            } else {
                print $formfile->showdocuments('Etatchangment', $subdir, $filedir, $urlsource, $genallowed, $delallowed, $modelpdf, 1, 0, 0, 40, 0);
            }
        }
        ...
        $upload_dir = DOL_DATA_ROOT . '/billanLaisse/Etatchangment/';
        $permissiontoadd = 1;
        $donotredirect = 1;
...
<html>
  ...
  <body>
  <center>
  <form method="POST" action="confirmEtatChangement.php">
    ...
    <table border="0" cellpadding="0" cellspacing="0" id="sheet0" class="sheet0 gridlines">
        ...
        <col class="col4">
        <center>
        <input type="date" name="EtatChangment46"  id="EtatChangment46" value="<?php if(isset($EtatChangment46)){ echo $EtatChangment46;} ?>"  placeholder ="Année" required style="margin-top: 18px;font-size:95%; background: #ededed; font-weight:bolder; padding: 8px 15px 8px 15px; border: block; ">
        <input type="hidden" name="check"  value="true">
        </center>
        <tbody>
          <tr class="row0">
            <td class="column0 style1 s style3" colspan="3">ETAT DES CHANGEMENTS DE METHODES</td>
          ...
          <tr class="row2">
            <td class="column0 style8 s">Nature des Changements</td>
            <td class="column1 style9 s">Justification des Changements</td>
            <td class="column2 style9 s">Influence des Changements sur le Patrimoine, la Situation Financière et les Résultats</td>
            
          </tr>
          <tr class="row3">
            <td class="column0 style11 s">I. Changements affectant les méthodes d'évaluation</td>
            <?php
            for ($i = 0; $i <=1; $i++) {
                echo '<td class="column1 style15 null"><input min="0" type="text" required style="width: 255px;" name="EtatChangment' . $i . '" id="EtatChangment' . $i . '" value="';
                if (isset(${"EtatChangment" . $i})) {
                    echo ${"EtatChangment" . $i};
                }
                echo '" /></td>' . "\n";
            }                 
            ?>
          ...
          <tr class="row11">
            <td class="column0 style17 s">II. Changements affectant les régles de présentation</td>
            <?php
            for ($i =23; $i <=24; $i++) {
                echo '<td class="column1 style15 null"><input min="0" type="text" required style="width: 255px;" name="EtatChangment' . $i . '" id="EtatChangment' . $i . '" value="';
                if (isset(${"EtatChangment" . $i})) {
