...
class pdf_Etatdesinterets extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = "", $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . \'/billanLaisse/Etatdesinterets/\';
			
				$year=GETPOST(\'year\');
			
				$file = $dir . "/Etatdesinterets".$year . ".pdf";
				// $file = $dir . "/Passif.pdf";
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset(\'ETAT DES INTERETS\'));
