...
// Title of page
print_barre_liste($langs->trans("List Reglement"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'bill', 0, '', '', $limit, 0, 0, 1);
...
while ($i < min($num, $limit)) {
	...
	if (!empty($arrayfields['p.ref']['checked'])) {
		$id=GETPOST("id");
		// Chemin vers le fichier PHP existant
		$fichier = DOL_DOCUMENT_ROOT . '\compta\paiement\class\paiement.class.php';
		...
		if (strpos($contenu, $code) === false) {
			
			$chemin_fichier = DOL_DOCUMENT_ROOT .'/core/modules/user/doc/pdf_Cheque.modules.php';
			// Contenu du fichier à créer
			
			require_once DOL_DOCUMENT_ROOT.'/custom/cheque/codepdf.php'; 
			...
			$contenu = str_replace('$url = DOL_URL_ROOT.\'/compta/paiement/card.php?id=\'.$this->id;', $code, $contenu);
	
			// Écriture du code dans le fichier
			file_put_contents($fichier, $contenu);
			
			$url = DOL_URL_ROOT . '/custom/cheque/paiement_list.php?id=' .$id;
			echo"<script>window.location.href='$url';</script>";
			
		}
	
		print '<td>'.$object->getNomUrl(5).'</td>';
	...
	if (!empty($arrayfields['p.amount']['checked'])) {
		...
		if (isset($totalarray['val']) && isset($totalarray['val']['amount'])) {
