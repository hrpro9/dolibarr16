...
class Paiement extends CommonObject
{
	 ...
	 */
	public $table_element = 'cheque_paiement';
	...
	/**
	 * @var string String with name of icon for paiement. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'paiement@cheque' if picto is file 'img/object_paiement.png'.
	 */
	...
	//  */
	// public $table_element_line = 'cheque_paiementline';
	...
	//  */
	// protected $childtablesoncascade = array('cheque_paiementdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->cheque->paiement->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->cheque->paiement->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->cheque->paiement->paiement_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->cheque->dir_output.'/paiement/'.$oldref;
				$dirdest = $conf->cheque->dir_output.'/paiement/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->cheque->dir_output.'/paiement/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'PAIEMENT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'PAIEMENT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'PAIEMENT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Paiement").'</u>';
		...
		$url = dol_buildpath('/cheque/paiement_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowPaiement");
		...
		$hookmanager->initHooks(array('paiementdao'));
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_paiement = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->CHEQUE_PAIEMENT_ADDON)) {
			$conf->global->CHEQUE_PAIEMENT_ADDON = 'mod_paiement_standard';
		}

		if (!empty($conf->global->CHEQUE_PAIEMENT_ADDON)) {
			$mybool = false;

			$file = $conf->global->CHEQUE_PAIEMENT_ADDON.".php";
			$classname = $conf->global->CHEQUE_PAIEMENT_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (!empty($conf->global->PAIEMENT_ADDON_PDF)) {
				$modele = $conf->global->PAIEMENT_ADDON_PDF;
			}
		}
