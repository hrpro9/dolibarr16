 ...
 */
class pdf_OrderDeVirement extends ModelePDFUser
{
    /**
    ...
    public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
		...
		if (1)
		{
			...
			else
			{
				...
				$dir = DOL_DATA_ROOT.'/grh/RH';
				
				//get date
				$dateg = $_SESSION['dateg'];

				$file = $dir."/".$object->login."_".$dateg."_OrderDeVirement.pdf";
			...
			if (file_exists($dir))
			{
				...
				// body
				$pdf->SetFont('', 'U', 36);
				$pdf->MultiCell(200,30,"Ordre de virement",0,"C");
				...
				$pdf->writeHTMLCell(180,10,$this->posxdesc + 3, $tab_top - 0,"Veuillez procéder au virement en faveur de <b>".$outputlangs->convToOutputCharset($object->lastname." ".$object->firstname)."</b>");
				$pdf->SetY($pdf->GetY() + 10);

				//set date of salary
				$dateLastDay=date("M Y", strtotime(date("d").'-'.$dateg));
				$pdf->writeHTMLCell(180,10,$this->posxdesc + 3, $tab_top + 15,"Libelle : <b> Salaire Mois ".$dateLastDay."</b>");

				$pdf->SetY($pdf->GetY() + 10);

				$pdf->MultiCell(0, 5, ''); // Set interline to 2
				...
				$pdf->SetY($pdf->GetY() + 10);
				$pdf->SetX($this->posxdesc + 3);
				$pdf->MultiCell(180, 2, $outputlangs->convToOutputCharset($Ubank->bank),0,'L');
				$pdf->SetX($this->posxdesc + 3);
				$pdf->MultiCell(180, 2,"RIB  ".$outputlangs->convToOutputCharset($Ubank->number),0,'L');

				if($user->salary-round($user->salary)!=0){
					$str="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Le montant de <b>".price($user->salary, 0, $outputlangs)." ".$currency
					." </b> &nbsp; &lt;&lt; <b>".$langs->getLabelFromNumber($user->salary,0|0)." ".$currency." ".$langs->getLabelFromNumber((abs($user->salary-round($user->salary)))*100,0|0)." Cents </b> &gt;&gt;";
				}
				else{
					$str="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Le montant de <b>".price($user->salary, 0, $outputlangs)." ".$currency
					." </b> &nbsp; &lt;&lt; <b>".$langs->getLabelFromNumber($user->salary,0|0)." ".$currency." </b> &gt;&gt;";
				}
				$pdf->writeHTMLCell(180,50,$this->posxdesc + 3, $tab_top + 40,$str);
				$pdf->SetY($pdf->GetY() + 10);

				$bankid = ($object->thirdparty->setPaymentMethods(1) <= 0 ? $conf->global->FACTURE_RIB_NUMBER : $object->fk_account);
				...
				$pdf->SetXY($curx, $cury);
				$pdf->writeHTMLCell(180,50,$this->posxdesc + 3, $tab_top + 60,"Le compte a débité est : <b>"
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$info= "Nom: ".$object->firstname." ".$object->lastname.". Mois: ".date("M").". Salaire: ".price($user->salary, 0, $outputlangs).$conf->currency
		.". Compte: ".$Ubank->number;
		...
		if ($object->date)
		{
			...
			$pdf->SetXY($posx, 45);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(100, 4,"Date.... ".dol_print_date($object->date, "day", false, $outputlangs, true), '', 'R');
		}
		else
		{
			$posy += 4;
