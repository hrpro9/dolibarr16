 ...
 */
class pdf_Dispatche extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			$name = (GETPOST('id', 'int') ? GETPOST('id', 'int') : GETPOST('orderid', 'int'));
			$importref=GETPOST('importref') ;
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . '/commande/dispatch/'.$importref.'/';
				$file = $dir . $importref . ".pdf";
			...
			if (file_exists($dir)) {
				...
				$sql .= " WHERE det.fk_commande = $name and p.fk_product_type=0";
				$res = $db->query($sql);
				if ($res->num_rows) {
				...
				foreach ($orders as $order) {
					$entrepot = $order[4];
					if (!in_array($entrepot, $entrepots)) {
						$entrepots[] = $entrepot;
					}
				}
				...
				foreach ($entrepots as $entrepot) {
					...
					foreach ($orders as $order) {
						if ($order[4] === $entrepot) {
							$quantiteLivrer = intval($order[2] < $order[3] ? $order[2] : $order[3]);
							$price = $quantiteLivrer * $order[1];
					...
					$table .= "<div class='details'>
								<h4>Total produits : " . $totalProducts . "</h4>
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont('', 'B', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "Dispatch commande :".$importref, 0, "C");
		...
		// $pdf->Ln(2);
		$pdf->MultiCell($w, 3, " Date de commande : " .  date("d/m/Y"), '', 'R');
		...
		$sql ="SELECT fk_soc FROM `llx_commande` WHERE rowid=$id";
		$rest=$db->query($sql);
		$param=$rest->fetch_assoc();
		$fc_soc=$param['fk_soc'];
		
		$sql_soc ="SELECT *  FROM `llx_societe` WHERE  rowid=$fc_soc";
		$rest_soc=$db->query($sql_soc);
		$param_soc=$rest_soc->fetch_assoc();
        $code_client=$param_soc["code_client"];
		$nom_soc=$param_soc["nom"];
		$address_soc=$param_soc["address"];
		$town_soc=$param_soc["town"];
		
		$tms=$param_soc["tms"];
		$date = date("d-m-Y", strtotime($tms));

		$posy += 4;
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->SetFontSize(9); // Set the font size to 9
		$pdf->MultiCell($w, 3, " Code client  : " . $code_client, '', 'R');
		...
		$sql_pro ="SELECT * FROM `llx_propal` WHERE  fk_soc=$fc_soc";
		$rest_pro=$db->query($sql_pro);
		$param_pro=$rest_pro->fetch_assoc();
        $ref=$param_pro["ref"];
		...
		$pdf->SetTextColor(0, 0, 60);
		$pdf->SetFontSize(9); 
		$pdf->MultiCell($w, 3," Réf. proposition commerciale : ". $ref .'/'.$date, '', 'R');
