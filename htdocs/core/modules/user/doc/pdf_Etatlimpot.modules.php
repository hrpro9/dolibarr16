...
class pdf_Etatlimpot extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = "", $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . '/billanLaisse/Etatlimpot/';
			
				$year=GETPOST('year');
			
				$file = $dir . "/Etatlimpot".$year . ".pdf";
				// $file = $dir . "/Passif.pdf";
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset('Etat impot'));
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont('', 'B', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "ETAT POUR LE CALCUL DE L IMPOT SUR LES SOCIETES - ENTREPRISES ENCOURAGEES ".$year, 0, "C");
