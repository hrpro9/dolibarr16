 ...
 */
class pdf_FichePersonnel extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (1) {
			...
			if ($object->specimen) {
				...
				$subdir = "/fiche";
				$dir = $conf->user->dir_output . $subdir;

				$file = $dir . "/" . $object->login . "_FichePersonnel.pdf";
			...
			if (file_exists($dir)) {
				...
				//Get All Info
				$sql = "SELECT status, cin, matricule, situation, enfants FROM llx_user_extrafields WHERE fk_object = $object->id";
				$res = $db->query($sql);
				if ($res->num_rows > 0) {
				...
				$sql = "SELECT param FROM llx_extrafields WHERE name = 'situation'";
				$res = $db->query($sql);
				if ($res->num_rows > 0) {
					$options = $res->fetch_assoc()['param'];
					$situationF = unserialize($options)['options'][$extra["situation"]];
				}

				$sql = "SELECT cnss, type from llx_Paie_UserInfo WHERE userid=$object->id";
				$res = $db->query($sql);
				if ($res->num_rows > 0) {
						...
						if ($param["auFiche"]) {
							...
							if ($resFiche->num_rows > 0) {
								...
								if ($fiche["checked"] == 1 || $fiche["amount"] > 0) {
									$indemimites[] = array("rub" => $param["rub"], "designation" => $param["designation"], "nombre" => "", "base" => $base, "taux" => $Tauxr, "apayer" => $apayer, "aretenu" => "", "surbulletin" => $param["surBulletin"]);
									$rubs .= $param["rub"] . ":indemimite:$apayer" . ";";
								}
							}
						} else {
							//if not au fiche
							...
							$indemimites[] = array("rub" => $param["rub"], "designation" => $param["designation"], "nombre" => "5", "base" => $base / 191, "taux" => $Tauxr, "apayer" => $apayer, "aretenu" => "", "surbulletin" => $param["surBulletin"]);
							$rubs .= $param["rub"] . ":indemimite:$apayer" . ";";
						}
					}
				}
				...
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 15, '<b>Nom et prenom:</b> ' . $object->lastname . " " . $object->firstname);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>Matricule:</b> ' . $matricule);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>CIN:</b> ' . $cin);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>Numero CNSS:</b> ' . $cnss);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>Adresse:</b> ' . $object->address);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>Situation familiale:</b> ' . $situationF);
				$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 12, '<b>Salaire de base:</b> ' . price($object->salary) . ' DH');

				foreach ($indemimites as $indemimite) {
					if ($indemimite["surbulletin"] == 1) {
						$pdf->writeHTMLCell(180, 30, $this->posxdesc + 3, $pdf->GetY() + 10, '<b>' . $indemimite["designation"] . ': </b> ' . price($indemimite["apayer"]) . ' DH');
					}
				}
