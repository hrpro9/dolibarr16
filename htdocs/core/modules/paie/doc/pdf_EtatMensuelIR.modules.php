 ...
 */
class pdf_EtatMensuelIR extends ModelePDFUser
{
	/**
	...
	{
		// phpcs:enable
		global $object, $user, $langs, $conf, $mysoc, $hookmanager, $nblines, $action, $prev_month, $prev_year, $periode, $year, $db;
		error_log("work?");
		...
		if (true) {
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset('Etat Mensuel IR'));
				$pdf->SetSubject($outputlangs->transnoentities("EtatIR"));
				...
				foreach ($salarais as $u) {
					$table .= '<tr>
						...
						<td class="center-td">' . $u["workingDays"] . '</td>
						<td class="center-td">' . price($u["netImposable"]) . '</td>
						<td class="center-td">' . price($u["ir"]) . '</td>
					</tr>';
				}
				...
				$table .= '<tr class="row-bordered row-content">
					...
					<td class="center-td">' . price(array_sum(array_column($salarais, 'netImposable'))) . '</td>
					<td class="center-td">' . price(array_sum(array_column($salarais, 'ir'))) . '</td>
				</tr>';
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont('', 'B', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "Etat Mensuel IR", 0, "C");
