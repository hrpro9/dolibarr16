 ...
 */
class pdf_EtatMensuelAMO extends ModelePDFUser
{
	/**
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams)
	{
		...
		if (true) {
			...
			if ($object->specimen) {
				...
				$dir = DOL_DATA_ROOT . '/grh/Etats/AMO';

				$file = $dir . "/i-gouvernancia_" . $name . "_EtatMensuelAMO.pdf";
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset('Etat Mensuel AMO'));
				...
				$pdf->MultiCell(200, 3, 'Etat Mensuel Assurance Maladie Obligatoire (AMO) Mois de ' . $french_months[$prev_month - 1] . ' ' . $prev_year, 0, 'C');
						...
						if (((object)$res1)->num_rows > 0) {
							...
							foreach (explode(";", $rubs) as $r) {
								...
								if ($rub[0] == $amoSalariale) {
									$u["amoSalariale"] = abs($rub[2]);
								} else if ($rub[0] == $amoPatronale) {
									$u["amoPatronale"] = abs($rub[2]);
								}
							}
						}
				...
				foreach ($salarais as $u) {
					$table .= '<tr>
						...
						<td class="center-td">' . price($u["amoSalariale"]) . '</td>
						<td class="center-td">' . price($u["amoPatronale"]) . '</td>
						<td class="center-td">' . price($u["amoSalariale"] + $u["amoPatronale"]) . '</td>
					</tr>';
					$total += $u["amoPatronale"] + $u["amoPatronale"];
				}
				...
				$table .= '<tr class="row-bordered row-content">
					...
					<td class="center-td">' . price(array_sum(array_column($salarais, 'amoSalariale'))) . '</td>
					<td class="center-td">' . price(array_sum(array_column($salarais, 'amoPatronale'))) . '</td>
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $totalNet, $periode)
	{
		...
		$pdf->SetFont('', 'B', $default_font_size + 3);
		$pdf->MultiCell(200, 2, "Etat Mensuel AMO", 0, "C");
