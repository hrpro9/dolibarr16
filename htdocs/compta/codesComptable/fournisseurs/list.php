...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($search_all != '' || $search_cti != '') && $action != 'list') {
	...
	if (!empty($conf->global->SOCIETE_ON_SEARCH_AND_LIST_GO_ON_CUSTOMER_OR_SUPPLIER_CARD)) {
		if ($obj->fournisseur > 0) {
			header("Location: ".DOL_URL_ROOT.'/compta/codesComptable/fournisseurs/card.php?id='.$id);
			exit;
		}
	}
...
print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'building', 0, "", '', $limit, 0, 0, 1);
...
while ($i < min($num, $limit)) {
	...
	if (!empty($arrayfields['s.nom']['checked'])) {
		...
		print "<a href='/compta/codesComptable/fournisseurs/card.php?id=$companystatic->id'>$companystatic->name</a>";
		print "</td>\n";
		if (!$i) {
			$totalarray['nbfield']++;
		}
