...
if ($action == 'updateSalary' && !$cancel) {
	...
	$cimr = $db->escape(GETPOST('numCimr', 'alpha'));
	...
	$sql = "REPLACE into llx_Paie_UserInfo(userid, cnss, mutuelle, cimr, type, mode_paiement) 
				values ($object->id, '$cnss','$mutuelle','$cimr', '$type', '$mode_paie');";
	$res = $db->query($sql);
	if ($res);
...
if ($action != 'edit' && $action != 'create' && $action != 'editSalary') {		// If not bank account yet, $account may be empty
	...
	if (
		!empty($conf->expensereport->enabled) &&
...
/*	print '<tr><td class="titlefield fieldrequired"> Type de Salaire </td>';
	...
	print '<td colspan="4"><select name="modePaiement">
			<option value=""></option>
			<option value="cheque" ' . (isset($modes["cheque"]) ? $modes["cheque"] : '') . '>Chéque</option>
			<option value="espece" ' . (isset($modes["espece"]) ? $modes["espece"] : '') . '>Espece</option>
			<option value="virement" ' . $modes["virement"] . '>Virement</option>
		    </select></td></tr>';
	...
	print '<tr><td class="titlefield fieldrequired"> N° CIMR </td>
		<td colspan="4"><input size="30" type="text" name="numCimr" value="' . $salaireInfo["cimr"] . '"></td>';
