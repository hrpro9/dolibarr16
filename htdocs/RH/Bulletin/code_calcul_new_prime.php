...
foreach ($rest as $paie_monthdeclaration) {
        ...
        if ($sn == $sn_newprime) {
            ...
            do {
                ...
                $sql = "SELECT situation, enfants, idpointage, matricule FROM " . MAIN_DB_PREFIX . "user_extrafields WHERE fk_object=" . $employe;
                $res = $db->query($sql);
                ...
                // Les cumule
                $sql = "SELECT sum(workingDays) as workingDays, sum(joursferie) as joursferie, sum(joursconge) as joursconge, sum(netImposable) as netImposable, 
                        sum(salaireBrut) as salaireBrut, sum(ir) as ir FROM llx_Paie_MonthDeclaration WHERE userid=$employe AND year=$annee_now AND month<=$mois_now";
                ...
                if (($comulWorkingDays + $workingdaysdeclaré) != 0) {
                    $irbase = (($comulnetImposable + $sni) / ($comulWorkingDays + $workingdaysdeclaré)) * ($params["workingDays"] * 12);
                }
                ...
                $ir_n = ($ir_n / ($params["workingDays"] * 12)) * ($comulWorkingDays + $workingdaysdeclaré);
                $ir_n = $ir_n - $comulIR;
