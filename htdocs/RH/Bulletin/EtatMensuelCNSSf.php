...
$res = $db->query($sql);
if (((object)$res)->num_rows > 0) {
    while ($_user = ((object)$res)->fetch_assoc()) {
        $id = $_user["userid"];
        ...
        if (((object)$res1)->num_rows > 0) {
            ...
            $_user["nom"] = $row1["lastname"];
            $_user["prenom"] = $row1["firstname"];
        }
        ...
        if (((object)$res1)->num_rows > 0) {
            ...
            foreach (explode(";", $rubs) as $r) {
                ...
                if ($rub[0] == $cnssSalariale) {
                    $_user["cnssSalariale"] = abs($rub[2]);
                } else if ($rub[0] == $cnssPatronale) {
                    $_user["cnssPatronale"] = abs($rub[2]);
                }
            }
        }
...
$table = '<table class="tableizer-table" style="margin: auto;">
    <thead>
        <tr class="row-bordered">
            ...
            <th>Salaire Plafoné</th>
            <th>Salaire Non Plafoné</th>
        </tr>
    </thead>
<tbody>';
...
foreach ($users as $_user) {
    if ($_user["salaireBrut"]<=6000)
    {
        $salaireplafond=$_user["salaireBrut"];
    ...
    $table .= '<tr>
        ...
        <td>' . $_user["workingDays"] . '</td>
        <td>' . price($_user["salaireBrut"], 0, '', 1, 1, 2) . '</td>
        <td>' . price($_user["cnssSalariale"], 0, '', 1, 1, 2) . '</td>
        <td>' . price($_user["cnssPatronale"], 0, '', 1, 1, 2) . '</td>
        <td>' . price($_user["cnssSalariale"] + $_user["cnssPatronale"], 0, '', 1, 1, 2) . '</td>
        <td>' . price($salaireplafond, 0, '', 1, 1, 2) . '</td>
        <td>' . price($_user["salaireBrut"], 0, '', 1, 1, 2) . '</td>
    </tr>';
    $total += $_user["cnssPatronale"] + $_user["cnssPatronale"];
}
$table .= '<tr class="row-bordered row-content">
    <td colspan="2"><b>Total général :</b></td>
    <td>' . array_sum(array_column($users, 'workingDays'), 0, '', 1, 1, 2) . '</td>
    <td>' . price(array_sum(array_column($users, 'salaireBrut')), 0, '', 1, 1, 2) . '</td>
    <td>' . price(array_sum(array_column($users, 'cnssSalariale')), 0, '', 1, 1, 2) . '</td>
    <td>' . price(array_sum(array_column($users, 'cnssPatronale')), 0, '', 1, 1, 2) . '</td>
    <td>' . price($total, 0, '', 1, 1, 2) . '</td>
</tr>';
