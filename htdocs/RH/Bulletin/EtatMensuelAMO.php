...
if ($action == "saveRubs") {
    ...
    $amoSalariale = (int)GETPOST("amoSalariale", 'int');
    $amoPatronale = (int)GETPOST("amoPatronale", 'int');
...
llxHeader("", "Etat Mensuel AMO");
$text = "Etat Mensuel AMO";
...
// GET les rubrique de l'AMO
$sql = "SELECT * FROM llx_Paie_EtatRubs WHERE id=1";
$res = $db->query($sql);
if (((object)$res)->num_rows > 0) {
    $row = ((object)$res)->fetch_assoc();
    $amoSalariale = $row["amoSalariale"];
    $amoPatronale = $row["amoPatronale"];
}

// Actions to build doc
$action = GETPOST('action', 'aZ09');
$upload_dir = DOL_DATA_ROOT . '/grh/Etats/AMO';
$permissiontoadd = 1;
$donotredirect = 1;
...
print '<form action="" method="post">
    ...
    <table>
        <tr>
            <td>La rebruque de la AMO Salariale</td>
            <td><input type="number" name="amoSalariale" id="amoSalariale" value="' . $amoSalariale . '"></td>
            <td rowspan="2"><input type="submit" value="enregistrer" class="button"></td>
        </tr>
        <tr>
            <td>La rebruque de la AMO Patronale</td>
            <td><input type="number" name="amoPatronale" id="amoPatronale" value="' . $amoPatronale . '"></td>
        </tr>
    </table>
    ...
    while ($user = ((object)$res)->fetch_assoc()) {
        ...
        if (((object)$res1)->num_rows > 0) {
            ...
            foreach (explode(";", $rubs) as $r) {
                //print $rub."<br>";
                $rub = explode(":", $r);
                if ($rub[0] == $amoSalariale) {
                    $user["amoSalariale"] = abs($rub[2]);
                } else if ($rub[0] == $amoPatronale) {
                    $user["amoPatronale"] = abs($rub[2]);
                }
            }
        }
...
print '<div style="text-align: center;">
    <h3>Etat Mensuel Assurance Maladie Obligatoire (AMO) Mois de ' . $french_months[$prev_month - 1] . ' ' . $prev_year . '</h3>';
...
foreach ($users as $user) {
    $table .= '<tr>
        ...
        <td>' . price($user["amoSalariale"]) . '</td>
        <td>' . price($user["amoPatronale"]) . '</td>
        <td>' . price($user["amoSalariale"] + $user["amoPatronale"]) . '</td>
    </tr>';
    $total += $user["amoPatronale"] + $user["amoPatronale"];
}
$table .= '<tr class="row-bordered row-content">
    ...
    <td>' . price(array_sum(array_column($users, 'amoSalariale'))) . '</td>
    <td>' . price(array_sum(array_column($users, 'amoPatronale'))) . '</td>
    <td>' . price($total) . '</td>
</tr>';
...
function GenerateDocuments()
{
    ...
    print '<input type="hidden" name="model" value="EtatMensuelAMO">';
...
function ShowDocuments()
{
    ...
    $subdir = '';
    $filedir = DOL_DATA_ROOT . '/grh/Etats/AMO/'.$subdir;
    ...
    print $formfile->showdocuments('EtatMensuelAMO', $subdir, $filedir, $urlsource, $genallowed, $delallowed, $modelpdf, 1, 0, 0, 40, 0, 'remonth=' . $month . '&amp;reyear=' . $prev_year, '', '', $societe->default_lang);
