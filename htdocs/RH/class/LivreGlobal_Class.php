...
for ($j = $start; $j < $start + 6; $j++) {
    ...
    $salaireMonsuels[$j] = $salaireMonsuelTot;
    $$salaireHoraires[$j] = $salaireHoraireTot;
    $primeDanciens[$j] = $primeDancienTot;
    $representations[$j] = $representationTot;
    $soldeConges[$j] = $soldeCongeTot;
    $allEnBrut[$j] = $enBruts;
    $primeCommercials[$j] = $primeCommercialTot;
    $salaireBruts[$j] = $brutGlobalTot;
    $allCotisations[$j] = $cotisations;

    $netImposables[$j] = $netImposableTot;
    $chargeFamilles[$j] = $chargeFamilleTot;
    $irNets[$j] = $irNetTot * -1;

    $allPasEnBrut[$j] = $pasEnBruts;
    $prev_arrondis[$j] = $prev_arrondiTot;
    $arrondis[$j] = $arrondiTot;
    $totalNets[$j] = $totalNetTot;
    $workingdaysdeclarés[$j] = $workingdaysdeclaréTot;
}
